...
export const reinfolibTool = createTool({
  ...
  inputSchema: z.object({
    priceClassification: z.string().length(2).regex(/^\d{2}$/).describe("Price classification code (2 digits)"),
    year: z.string().length(4).regex(/^\d{4}$/).describe("Target year (YYYY)"),
    quarter: z.string().length(1).regex(/^[1-4]$/).describe("Target quarter (1-4)"),
    area: z.string().length(2).regex(/^\d{2}$/).optional().describe("Area code (2 digits)"),
    city: z.string().length(5).regex(/^\d{5}$/).optional().describe("City code (5 digits)"),
    station: z.string().length(6).regex(/^\d{6}$/).optional().describe("Station code (6 digits)"),
