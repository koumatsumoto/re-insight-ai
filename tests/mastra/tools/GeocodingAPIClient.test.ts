...
describe("GeocodingAPIClient", () => {
  ...
  it("should fetch and parse geocoding data successfully", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining(
        "https://maps.googleapis.com/maps/api/geocode/json?address=%E7%AF%89%E5%9C%B06%E4%B8%81%E7%9B%AE",
      ),
    );
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("key=dummy-api-key"));
  ...
  it("should handle API errors", async () => {
    ...
    await expect(client.searchAddress("invalid-address")).rejects.toThrow("Failed to fetch geocoding data: Not Found");
  });

  it("should handle network errors", async () => {
    (global.fetch as any).mockRejectedValueOnce(new Error("Network error"));

    await expect(client.searchAddress("test-address")).rejects.toThrow("Network error");
  });
  ...
  it("should throw ZodError for invalid response format", async () => {
    ...
    await expect(client.searchAddress("test-address")).rejects.toThrow(ZodError);
  });
});
