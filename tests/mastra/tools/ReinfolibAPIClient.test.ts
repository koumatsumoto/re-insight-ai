import {
  ...
  RealEstateQueryParams,
} from "../../../src/mastra/tools/ReinfolibAPIClient";
...
describe("ReinfolibAPIClient", () => {
  ...
  it("should handle API errors", async () => {
    ...
    await expect(client.getRealEstateTransaction(params)).rejects.toThrow(
      "Failed to fetch real estate data: Unauthorized",
    );
  });
  ...
  it("should handle network errors", async () => {
    ...
    await expect(client.getRealEstateTransaction(params)).rejects.toThrow("Network error");
  });
  ...
  it("should throw ZodError for invalid response format", async () => {
    ...
    await expect(client.getRealEstateTransaction(params)).rejects.toThrow(ZodError);
  });
  ...
  it("should validate query parameters", async () => {
    ...
    await expect(client.getRealEstateTransaction(invalidParams as any)).rejects.toThrow(ZodError);
  });
  ...
  it("should require at least one location parameter", async () => {
    ...
    await expect(client.getRealEstateTransaction(paramsWithoutLocation)).rejects.toThrow(
      "At least one of area, city, or station must be specified",
    );
  });
});
